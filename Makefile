# This Makefile is for the Parallel::Simple::Dynamic extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Parallel/Simple/Dynamic.pm]
#     AUTHOR => q[Aleksandra Markovets <marsa@cpan.org>]
#     BUILD_REQUIRES => {  }
#     LICENSE => q[perl]
#     NAME => q[Parallel::Simple::Dynamic]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0] }
#     VERSION_FROM => q[lib/Parallel/Simple/Dynamic.pm]
#     clean => { FILES=>q[Parallel-Simple-Dynamic-*] }
...
NAME_SYM = Parallel_Simple_Dynamic
VERSION = undef
...
MAKE = make
FULLEXT = Parallel/Simple/Dynamic
BASEEXT = Dynamic
PARENT_NAME = Parallel::Simple
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Parallel/Simple/Dynamic.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Parallel/Simple
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Parallel/Simple
...
TO_INST_PM = lib/Parallel/Simple/Dynamic.pm

PM_TO_BLIB = lib/Parallel/Simple/Dynamic.pm \
...
DISTNAME = Parallel-Simple-Dynamic
DISTVNAME = Parallel-Simple-Dynamic-undef
...
manifypods : pure_all  \
	lib/Parallel/Simple/Dynamic.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Parallel/Simple/Dynamic.pm $(INST_MAN3DIR)/Parallel::Simple::Dynamic.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               Parallel-Simple-Dynamic' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            undef' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           dynamically splits a big list of data into several parts for processing them in parallel ' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Aleksandra Markovets <marsa@cpan.org>' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>dynamically splits a big list of data into several parts for processing them in parallel </ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Aleksandra Markovets &lt;marsa@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Parallel/Simple/Dynamic.pm blib/lib/Parallel/Simple/Dynamic.pm 
